...
namespace UHFReader288Demo
{
    public partial class Form1 : Form
    {
        ...
        private IntPtr g_ptrWnd = IntPtr.Zero;
        /// <summary>
        /// Device Search的回调函数;
        /// </summary>
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            Maintab.TabPages.Remove(Main_Page5);
            Maintab.TabPages.Remove(Main_Page6);
            ...
            for (i = 0; i < 101; i++)
            {
                cbb_range.Items.Add(i + "");
            }
            cbb_range.SelectedIndex = 100;
            lbModuleDescribe.Text = "";
        }
        ...
        private void btConnect232_Click(object sender, EventArgs e)
        {
            ...
            if (fCmdRet != 0)
            {
                if (fBaud == 6)//第一次用115200打开的，再尝试下57600
                {
                    fBaud = 5;
                    fCmdRet = RWDev.OpenComPort(portNum, ref fComAdr, fBaud, ref FrmPortIndex);
                    if (fCmdRet != 0)
                    {
                        strLog = "Connect reader failed: " + GetReturnCodeDesc(fCmdRet);
                        WriteLog(lrtxtLog, strLog, 1);
                        return;
                    }
                    else
                ...
                else if (fBaud == 5)//第一次用57600打开的，再尝试下115200
                {
                    fBaud = 6;
                    fCmdRet = RWDev.OpenComPort(portNum, ref fComAdr, fBaud, ref FrmPortIndex);
                    if (fCmdRet != 0)
            ...
            strLog = "Connected: " + ComboBox_COM.Text + "@" + ComboBox_baud2.Text;
            WriteLog(lrtxtLog, strLog, 0);
            ComboBox_baud.SelectedIndex = ComboBox_baud2.SelectedIndex;
            //处理界面元素是否有效
            EnabledForm();
        ...
        private void btFlashCl_Click(object sender, EventArgs e)
        {
            ...
            ////实时查询刷屏
            if (tabControl1.SelectedTab == tabPage_Module)
            {
                lxLed_Mtag.Text = "0";
                lxLed_Mtime.Text = "0";
                total_time = System.Environment.TickCount;
        ...
        private void btGetInformation_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string RDVersion = "";
                string ChipType = "";
                int ret = GetRDVersionAntennaChipType(ReaderType, ref RDVersion, ref AntennaNum, ref ChipType);
                if (ret == 0)
                {
                    text_RDVersion.Text = RDVersion + "--" + Convert.ToString(VersionInfo[0], 10).PadLeft(2, '0') + "." + Convert.ToString(VersionInfo[1], 10).PadLeft(2, '0');
                    ...
                    if (ChipType == "TM600")
                    {
                        ...
                        if (Maintab.TabPages.IndexOf(Main_Page3) == -1)
                            Maintab.TabPages.Add(Main_Page3);
                        if (Maintab.TabPages.IndexOf(Main_Page5) == -1)
                            Maintab.TabPages.Add(Main_Page5);
                        if (Maintab.TabPages.IndexOf(Main_Page6) == -1)
                            Maintab.TabPages.Add(Main_Page6);
                    ...
                    if (AntennaNum == 1)
                    {
                        ReadTypes = Convert.ToString(ReaderType, 16).PadLeft(2, '0').ToUpper();
                        //模块版本号显示
                ...
                if (ChipType == "EX10")
                {
                    byte[] DescribeInfo = new byte[16];
                    int Result = RWDev.GetModuleDescribe(ref fComAdr, DescribeInfo, frmcomportindex);
                    if (Result == 0)
                    {
                        byte CalType = 0;
                        byte[] IntVersion = new byte[4];
                        byte[] BootVersion = new byte[2];
                        byte[] BordVersion = new byte[2];
                        byte[] ARMType = new byte[2];
                        byte ModuleType = 0;
                        CalType = DescribeInfo[0];
                        Array.Copy(DescribeInfo, 1, IntVersion, 0, 4);
                        Array.Copy(DescribeInfo, 6, BootVersion, 0, 2);
                        Array.Copy(DescribeInfo, 8, BordVersion, 0, 2);
                        Array.Copy(DescribeInfo, 10, ARMType, 0, 2);
                        ModuleType = DescribeInfo[12];
                        if (BordVersion[0] != 0 && BordVersion[1] != 0)
                        {
                            lbModuleDescribe.Text = Convert.ToString(IntVersion[0], 10).PadLeft(2, '0') + "." + Convert.ToString(IntVersion[1], 10).PadLeft(2, '0')
                            + "." + Convert.ToString(IntVersion[2], 10).PadLeft(2, '0') + "." + Convert.ToString(IntVersion[3], 10).PadLeft(2, '0')
                            + "-"
                            + CalType
                            + "-"
                            + Convert.ToString(BootVersion[0], 10).PadLeft(2, '0') + "." + Convert.ToString(BootVersion[1], 10).PadLeft(2, '0')
                            + "-"
                            + Convert.ToString(BordVersion[0], 10).PadLeft(2, '0') + "." + Convert.ToString(BordVersion[1], 10).PadLeft(2, '0')
                            + "-"
                            + Encoding.ASCII.GetString(ARMType, 0, 2)
                            + "-"
                            + ModuleType;
                        }
                        else
        ...
        private int GetRDVersionAntennaChipType(byte ReaderType, ref string RDVersion, ref int AntennaNum, ref string ChipType)
        {
            int retlen = 0;
            string path = ".\\ReaderType.ini";
            if (File.Exists(path))  //检查是否有配置文件，并且配置文件内是否有相关数据。
            {
                StringBuilder RDVersionStr = new StringBuilder(32);
                StringBuilder AntennaNumStr = new StringBuilder(4);
                StringBuilder ChipTypeStr = new StringBuilder(16);

                //获取型号
                retlen = GetPrivateProfileString("0x" + ReaderType.ToString("X2").ToUpper(), "RDVersion", "", RDVersionStr, 32, path);
                if (retlen == 0)
                {
                    MessageBox.Show("ReaderType.ini Not found reader type!");
                    return -1;
                }
                else
                {
                    RDVersion = RDVersionStr.ToString();
                }

                //获取天线数量
                retlen = GetPrivateProfileString("0x" + ReaderType.ToString("X2").ToUpper(), "AntennaNum", "", AntennaNumStr, 4, path);
                if (retlen == 0)
                {
                    MessageBox.Show("ReaderType.ini not match antenna number!");
                    return -1;
                }
                else
                {
                    AntennaNum = Convert.ToInt32(AntennaNumStr.ToString());
                }

                //获取芯片类型
                retlen = GetPrivateProfileString("0x" + ReaderType.ToString("X2").ToUpper(), "ChipType", "", ChipTypeStr, 16, path);
                if (retlen == 0)
                {
                    MessageBox.Show("ReaderType.ini not match reader chip!");
                    return -1;
                }
                else
                {
                    ChipType = ChipTypeStr.ToString();
                }

            }
            else
            {
                MessageBox.Show("ReaderType.ini not found!");
                return -1;
            }
        ...
        private void btBaudRate_Click(object sender, EventArgs e)
        {
            ...
            {
                ComboBox_baud2.SelectedIndex = ComboBox_baud.SelectedIndex;
                string strLog = "Set baud rate success ";
                WriteLog(lrtxtLog, strLog, 0);
            }
        }
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            if (button9.Text=="Start")
            {
                ...
                if (Session >= 0 && Session < 4)
                {
                    for (int m = 0; m < 4; m++)
                    {
                        fCmdRet = RWDev.SelectCMDByAntenna(ref fComAdr, SelectAntenna, AntennaNum, Session, ACTION, MaskMem, MaskAdr, MaskLen, MaskData, 0, frmcomportindex);
                        Thread.Sleep(5);
                    }
                }
                else if (Session == 254)
                {
                    for (int m = 0; m < 2; m++)
                    {
                        fCmdRet = RWDev.SelectCMDByAntenna(ref fComAdr, SelectAntenna, AntennaNum, 2, 0, MaskMem, MaskAdr, MaskLen, MaskData, 0, frmcomportindex);
                        Thread.Sleep(5);
                    }
                    for (int m = 0; m < 2; m++)
                    {
                        fCmdRet = RWDev.SelectCMDByAntenna(ref fComAdr, SelectAntenna, AntennaNum, 3, 0, MaskMem, MaskAdr, MaskLen, MaskData, 0, frmcomportindex);
                        Thread.Sleep(5);
                    }
                }
        ...
        private void button21_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                UdpClient udpclient = new UdpClient(11000 + RandomNumber);
                ...
                IPEndPoint iep1 = new IPEndPoint(IPAddress.Any, 0);//初始化一个发送广播和指定端口的网络端口实例
                ...
                for (int i = 0; i < 255; i++)
                {
                    ...
                    if (m_count > 0)
                    {
                        ...
                        {
                            string[] temp = fRecvIDPstring.Split('/');
                            if (temp.Length >= 6)
                            {
                                Information.mac = temp[0];
                                Information.portnum = temp[1];
                                Information.usename = temp[4];
                                Information.dsname = temp[5];
            ...
            try
            {
                    ...
                    && listView1.SelectedIndices[0] != -1)
                {
                    ...
                    while (i < 3)
                    {
                        ...
                        catch (Exception ex)//指定IP发送出错，尝试广播
                        {
        ...
        private void button16_Click(object sender, EventArgs e)
        {
            ...
            cfgData[0] = cfgData[1] = cfgData[2] = 0;
            cfgData[3] = (byte)cbb_range.SelectedIndex;
            int len = 4;
            fCmdRet = RWDev.SetCfgParameter(ref fComAdr, (byte)0, cfgNo, cfgData, len, frmcomportindex);
            if (fCmdRet != 0)
            {
                string strLog = "";
        ...
        private void button15_Click(object sender, EventArgs e)
        {
            ...
            byte[] cfgData = new byte[4];
            int len = 0;
            fCmdRet = RWDev.GetCfgParameter(ref fComAdr, cfgNo, cfgData, ref len, frmcomportindex);
            if (fCmdRet == 0 && len == 4)
            {
                if (cfgData[3] < 101)
                    cbb_range.SelectedIndex = cfgData[3];
                string strLog = "Get success ";
                WriteLog(lrtxtLog, strLog, 0);
            }
            else
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                    ...
                    && listView1.SelectedIndices[0] != -1)
                {
                    ...
                    Information.IP = "192.168.1.190";
                    Information.gi = "192.168.1.1";//网关
                    Information.nm = "255.255.255.0";

                    Information.bb = "1";
                    Information.br = "7";//115200
                    Information.di = "127.0.0.1";
                    Information.dp = "1001";
                    Information.dsname = "";
                    Information.dt = "0";
                    Information.fc = "1";
                    Information.portnum = "6000";
                    Information.pr = "0";
                    Information.rm = "0";
                    Information.tup = "1";
                    Information.usename = "";
                    ...
                    request = "SON" + Information.usename + "|18";//用户名
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);

                    request = "SDN" + Information.dsname + "|19";//设备名
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);

                    request = "STP" + Information.tup + "|20";//设置传输协议,1-tcp
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);

                    request = "SPN" + Information.portnum + "|21";//设备端口号
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);

                    request = "SRM" + Information.rm + "|22";//0-服务器
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);

                    request = "SFC" + Information.fc + "|23";//流向控制
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);

                    request = "SDT" + Information.dt + "|24";//DTR设置
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);

                    request = "SBR" + Information.br + "|25";//波特率设置
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);

                    request = "SPR" + Information.pr + "|26";//校验位设置
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);

                    request = "SBB" + Information.bb + "|27";//比特位
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);
                    ...
                    request = "SCM" + Information.cm + "|29";//设备作为客户端时连接服务器的方式
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);

                    request = "SCT" + Information.ct + "|30";//连接超时时限
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);

                    request = "SML" + Information.ml + "|31";//最大数据打包的长度
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);

                    request = "SMD" + Information.md + "|32";//最小字符间延时
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);

                    request = "SDI" + Information.di + "|33";//目标IP地址
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);

                    request = "SDP" + Information.dp + "|34";//目标端口号
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);

                    request = "SGI" + Information.gi + "|35";//网关IP地址
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);

                    request = "SNM" + Information.nm + "|36";//网络掩码
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);

                    request = "SIP" + Information.IP + "|37";//设置设备IP
                    buffer = Encoding.ASCII.GetBytes(request);
                    sock.SendTo(buffer, iep);
                    Thread.Sleep(10);
